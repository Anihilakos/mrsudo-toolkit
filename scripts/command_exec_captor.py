...
def notify_fsm(cr_event_id, progression_link):
    url = progression_link+"/event"
...
def cmd_execution_verification(line, verification_criterion_dict_list):
    ...
    for verification_criterion_dict in verification_criterion_dict_list:
        command = get_command_cmd_execution(line)
        user = get_user_cmd_execution(line)
        with open('/home/vagrant/demo_log.txt', 'a+') as file:
            #file.write("--------------------------------------------------")
            #file.write(verification_criterion_dict.__str__())
            # file.write(verification_criterion_dict['user'] == user)
            # file.write(verification_criterion_dict['command'] + " vs " + command)
            print(verification_criterion_dict)
            print(verification_criterion_dict['user'] == user)
            print(verification_criterion_dict['command'] == command.strip())
            ...
            if verification_criterion_dict['user'] == user and verification_criterion_dict['command'] == command.strip():
                ...
                # dummy_notify_fsm(verification_criterion_dict['cr_event'])
                # file.write(verification_criterion_dict.__str__())
                file.write("\n")
                file.write("User event verified, send event with ID ->" + str(verification_criterion_dict['cr_event']) + " to FSM")
